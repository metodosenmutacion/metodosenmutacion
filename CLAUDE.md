...
Implementado en Mega-Batch 4:
- Portal de subida interno en /enviar/ implementado vía Netlify Function (netlify/functions/submit-method.ts) protegido por contraseña ADMIN_PASSWORD y autenticado en GitHub por GITHUB_TOKEN (ambas variables configuradas manualmente en Netlify dashboard). Commit atómico final vía Git Data API (create blob → get ref → get tree → create tree → create commit → update ref). Para v0.2 se sustituirá por flujo público con verificación por email.
...
- Cartografía v0.1.3: secuencia de Weyl con proporción áurea (0.6180339887 / 0.3819660113) para distribuir nodos uniformemente sin clustering. Rango efectivo 88% horizontal × 82% vertical. Los nodos con territorio COMÚN se anclan en posición fija (top:40, left:42). Canvas ampliado a min-height:90vh.

...
- Portal de subida interno en /enviar/ implementado vía Netlify Function (netlify/functions/submit-method.ts) protegido por contraseña ADMIN_PASSWORD y autenticado en GitHub por GITHUB_TOKEN (ambas variables configuradas manualmente en Netlify dashboard). El portal escribe directamente al repo: src/content/methods/[slug]/method.md y archivos de documentación. Netlify redespliega automáticamente al detectar el commit. Tiempo total de subida: ~1-3 minutos por método. Limitación: max ~5MB de archivos por método (límite payload de Functions). Para v0.2 se sustituirá por flujo público con verificación por email.
